...
def explore_from_oscar(model, out_filename) :
	...
	prog = 0
	if os.path.exists('dataset/progress.txt'):
		with open('dataset/progress.txt') as f:.
	...
	for i, d in enumerate(tqdm(oscardata)):
		try :
				...
				for j in range(0, len(texts)) :
					if len(preds[j]) > 0 :
						...
						if len(df_refs.loc[ df_refs['id'] == iddoc ]) == 0 :
							path = "dataset/files/doc_" + str(iddoc) + ".txt"
			...
			with open('dataset/progress.txt', 'w') as f:
				f.write(f'{i + prog}')
			...
			if len(parts) > 1 :
				...
				err_out = out
			df.to_csv('dataset/' + err_out, mode='w', index=False)
			df_refs.to_csv('dataset/docs_ref.csv', mode='w', index=False)
			raise
