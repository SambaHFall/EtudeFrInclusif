...
"""
build corpus final fine from an annotated "exploration.csv" like file
"""
...
def build_dataset(input_filename, out_filename="new_corpus.csv", loadspacyfrom="fr_core_news_sm") :
	...
	doc_clusters = []
	with open(sur_script_dir + "/dataset/same_files.txt") as f :
		lines  = f.readlines()
		for line in lines :
			clust = eval( line[:-1] )
			clust = [ re.split(r"[_\.]", item)[1] for item in clust ]
			doc_clusters.append( {"rep" : None, "clust" : clust } )
	...
	for i, content in tqdm(input_df.iterrows()) :
		...
		if content["annotation"] :
			iddoc = content["iddoc"]
			superid= iddoc.split('_')[0]

			it = 0
			while it < len(doc_clusters) and superid not in doc_clusters[it]["clust"] :
				it += 1 

			if it < len(doc_clusters) :
				if doc_clusters[it]["rep"] is not None and doc_clusters[it]["rep"] != superid:
					continue
				else :
					doc_clusters[it]["rep"] = superid
			...
			#parfois, il passe des list au str : on retransforme en list
			if isinstance(row["spans"], str) :
				row["spans"] = eval(row["spans"]) 
			if isinstance(row["labels"], str) :
				row["labels"] = eval(row["labels"]) 
				


			rg = eval(content["range"])
			sp = (rg['beg'], rg['end']) 
			if str(sp) not in [str(item) for item in row["spans"] ] :
				row["spans"].append(sp)
...
build_dataset("corrected_exploration_ann.csv")
