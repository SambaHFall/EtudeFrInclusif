...
class CRFModel(AnnPredModel):
    ...
    def tk_to_vect(self, tk):
        raw = unidecode(tk.text)
        return [self.nb_non_alpha(raw),#number of characters which are neither lower case characters nor numerals
                1 if not self.fr_dict.exists(raw.lower()) else 0,#1/0 whether the word is present in a french dictionnary
                self.nb_num(tk.text), #number of numerals
    ...
    def tks_to_features(self, tks):
        return self.vects_to_features([self.tk_to_vect(tk) for tk in tks])
    ...
    def _sub_crf_shaping(self, text: str, anns: list[Ann]) -> pd.DataFrame:
        ...
        dfdict["feats"].append(self.tks_to_features(dfdict["tokens"][0]))
